...
data.frame(number_of_solar_panels = production_values, manufacturing_cost = round(predicted_costs, 2))
```
...
conf_int <- confint(model)["Log_number_of_solar_panels", ]

# Lower and Upper Bound for experience parameter (b1)
b1_LB <- conf_int[1]  # Lower Bound
b1_UB <- conf_int[2]  # Upper Bound
```
...
log_cost_LB <- log_predicted_costs + (b1_LB - b1) * log(production_values)  # Adjust for LB
log_cost_UB <- log_predicted_costs + (b1_UB - b1) * log(production_values)  # Adjust for UB
 ...
 **A bid closer to $598.54 would maximize competitiveness**, making Heliotronics a strong contender in the Swiss market.  
 **A bid near $792.72 provides a buffer against cost uncertainties**, ensuring sustainable profit margins.  
 **Strategic Recommendation:** To balance **competitiveness and profitability**, **Heliotronics should place a bid between $598.54 and $792.72**, considering market conditions and pricing strategy.
